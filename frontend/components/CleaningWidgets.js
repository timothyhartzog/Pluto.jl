...
function StringsWidget({ on_insert }) {
    ...
    const handle_case_change = useCallback(
        (type, checked) => {
            if (type === "lowercase") {
                set_lowercase(checked)
                if (checked) { set_uppercase(false); set_titlecase(false) }
            }
            if (type === "uppercase") {
                set_uppercase(checked)
                if (checked) { set_lowercase(false); set_titlecase(false) }
            }
            if (type === "titlecase") {
                set_titlecase(checked)
                if (checked) { set_lowercase(false); set_uppercase(false) }
            }
        },
        []
    )
